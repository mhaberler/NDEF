...
public:
  ...
  NfcTag(const Uid& uid, TagType tagType);
  NfcTag(const Uid& uid, TagType tagType, bool isFormatted);
  NfcTag(const Uid& uid, TagType tagType, NdefMessage &ndefMessage);
  NfcTag(const Uid& uid, TagType tagType, const byte *ndefData, const uint16_t ndefDataLength);
  ~NfcTag(void);
