...
NdefMessage::NdefMessage(const byte *data, const uint16_t numBytes) {
  ...
  while (index < numBytes) {
    ...
    if (sr) {
      ...
      payloadLength = (static_cast<uint32_t>(data[index]) << 24) | (static_cast<uint32_t>(data[index + 1]) << 16) | (static_cast<uint32_t>(data[index + 2]) << 8) | static_cast<uint32_t>(data[index + 3]);
      index += 3;
    }
...
#ifdef NDEF_USE_SERIAL
    Serial.println(
      F("WARNING: Too many records. Increase MAX_NDEF_RECORDS."));
#endif
    return false;
  }
}

void NdefMessage::addMimeMediaRecord(const char *mimeType,
                                     const char *payload) {
  addMimeMediaRecord(mimeType, (uint8_t *)payload, strlen(payload) + 1);
}

void NdefMessage::addMimeMediaRecord(const char *mimeType, byte *payload,
...
void NdefMessage::addExternalRecord(const char *type, const byte *payload,
                                    const uint16_t payloadLength) {
  NdefRecord r;
...
bool NdefMessage::toJson(JsonDocument &result) {
  JsonArray records = result.to<JsonArray>();
  for (auto i = 0; i < _recordCount; i++) {
    NdefRecord nr = getRecord(i);
    const JsonObject &jr = records.createNestedObject();
    jr["tnf"] = (int)nr.getTnf();
    if (nr.getTypeLength() > 0) {
      jr["type"] = String(nr.getType(), nr.getTypeLength());
    }
    if (nr.getIdLength() > 0) {
      jr["id"] = String(nr.getId(), nr.getIdLength());
    }
    if (nr.getPayloadLength() > 0) {
      jr["payload"] = String(nr.getPayload(), nr.getPayloadLength());
    }
  }
  return true;
}
